<template>
    ...
    <div class="create-agreement" v-if="!isUpdating">
      ...
      <form @submit.prevent="createRentalAgreement">
        <input type="number" v-model="newAgreement.renter_id" placeholder="Renter ID" required />
        <input type="number" v-model="newAgreement.property_id" placeholder="Property ID" required />
        <label for="start-date"><strong>Start Date: </strong></label>
        <input id="start-date" type="date" v-model="newAgreement.start_date" required />

        <label for="end-date"><strong>End Date: </strong></label>
        <input id="end-date" type="date" v-model="newAgreement.end_date" required />
        <input type="number" v-model="newAgreement.monthly_rent" placeholder="Monthly Rent" required />
        <input type="number" v-model="newAgreement.deposit_amount" placeholder="Deposit Amount" required />
        <button id="create-agreement-button" type="submit">Create Agreement</button>
      </form>
    </div>
    ...
    <div class="update-agreement" v-if="editAgreementId !== null">
      ...
      <form @submit.prevent="updateRentalAgreement">
        ...
        <input type="number" v-model="newAgreement.monthly_rent" placeholder="Monthly Rent" required />
        <input type="number" v-model="newAgreement.deposit_amount" placeholder="Deposit Amount" required />
        <button id="update-agreement-form-button" type="submit">Update Agreement</button>
        <button id="cancel-form-button" type="button" @click="cancelUpdate">Cancel</button>
      </form>
    </div>
    ...
    <div v-if="isAdmin" class="agreements-container">
      <div v-for="agreement in agreements" :key="agreement.rental_agreement_id" class="agreement-card">
        ...
        <button id="update-agreement-button" @click="openUpdateModal(agreement.rental_agreement_id)">Update Agreement</button>
        <button id="delete-agreement-button" @click="deleteRentalAgreement(agreement.rental_agreement_id)">Delete Agreement</button>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      openUpdateModal(agreementId) {
        const agreementToUpdate = this.agreements.find(agreement => agreement.rental_agreement_id === agreementId);
  ...
  AgreementService.updateRentalAgreement(this.editAgreementId, this.newAgreement)
    .then((response) => {
      alert("Agreement updated successfully.");
      const index = this.agreements.findIndex(agreement => agreement.rental_agreement_id === this.editAgreementId);
        ...
        if (confirm("Are you sure you want to delete this agreement?")) {
          AgreementService.deleteRentalAgreement(agreementId)
            .then(() => {
              ...
              this.agreements = this.agreements.filter(a => a.rental_agreement_id !== agreementId); // Remove agreement from list
            })
            .catch((error) => {
